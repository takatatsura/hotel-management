    ...
    streams: [
        ...
        },
        {
            ...
            path: path.join(
                __dirname,
                './',
                nconf.get('logging:dir'),
                `${process.env.NODE_ENV}-${nconf.get('server:name')}.log`
            ),
        },
    ],
});
...
const cors = corsMiddleware({
    ...
    origins: ['*'],
    allowHeaders: [
        'Origin, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version, X-Response-Time, X-PINGOTHER, X-CSRF-Token, Authorization, API-Token',
    ],
...
const registerRoute = (route) => {
    ...
    route.meta.paths.forEach((aPath) => {
        const routeMeta = {
            name: routeName,
            path: aPath,
...
// Setup middleware
['bookings', 'guests', 'rooms'].forEach(setupMiddleware);
